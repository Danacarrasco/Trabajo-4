...
```{r}
proc_data %>% 
  ggplot(aes(x = sexo, y = horas_semanales)) + 
  geom_bar(stat = "identity", fill = "pink") +
  ggtitle("Cantidad de horas semanales que trabajo segun sexo")
```

```{r}
proc_data %>% 
  ggplot(aes(x = sexo, y = ingresos)) + 
  geom_bar(stat = "identity", fill = "pink")
  ggtitle("Tngresos liquidos segun sexo")
```
...
```
```{r}
proc_data$sexo_numerica <- as.numeric(proc_data$sexo)
```
```{r}
proc_data <- proc_data %>% select(sexo_numerica, trabajo_sinquehacer, horas_semanales, ingresos, activ, busco_trabajo, razon_nobuscar)
```
...
La correlación más alta es entre las variables "activ" y "trabajo_sinquehacer", con una correlación de 0.903. Lo que indica que hay una fuerte asociación entre estar activo y trabajar sin realizar actividades domésticas.Además, también existe una correlación moderada entre las variables "sexo_numerica" y "trabajo_sinquehacer" de 0.218, al igual que entre las variables "activ" y "sexo_numerica" de 0.221
...
```{r}
sjPlot::plot_scatter(proc_data, razon_nobuscar, sexo_numerica)
```

```{r}
sjPlot::plot_scatter(proc_data, sexo_numerica, ingresos)
...
```{r}
graph1 <- ggplot(proc_data, aes(x = ingresos, y = sexo_numerica)) +
  geom_point(size = 1) +  
  geom_smooth(method = "lm", se = FALSE) +  
  labs(x = "ingresos", y = "sexo")  

```


```{r}
graph2 <- ggplot(proc_data, aes(x = trabajo_sinquehacer, y = sexo_numerica)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Trabajar sin realizar act domestica", y = "Sexo")
ggarrange(graph1, graph2, nrow = 1) 
```
Sexo e ingresos tienen una pendiente negativa, con bastante fuerza. mientras que Sexo y trabajar sin realizar labores domesticas, tienen pendiente ligeramente positiva.


TRABAJO 4

Hipotesis: mientras mayores sean las horas que una mujer dedicada al trabajo domestico se obtiene un impacto negativo en los ingresos disminuyendo los mismos. 

```{r}
#Regresion Simple

modelo0 = lm(ingresos ~ sexo_numerica,
             data = proc_data)
...
```
La tabla nos muestra un intercepto de ingresos promedio para un hombre despues de imputar valores con la mediana es  de  1,056,855.99 si las demas variables se mantienen constantes , mientras que el coeficiente sexo_numerica, nos muestra que para sexo= 1 que corresponde a las mujeres,los ingresos son 228,861.91 menores en comparacion con el grupo 0, Hombres. 
Con un R2 de 0.027 solo se explica un 2.7% en la variabilidad del ingreso por lo que se propone incluir mas variables en el modelo.


```{r}
#Regresion Multiple
modelo1 = lm(ingresos ~ sexo_numerica + trabajo_sinquehacer, 
             data = proc_data)
...
```
El intercepto del modelo es de 1,098,991.50. es decir que para el sujeto de referencia (hombre), el salario líquido en su trabajo se estima en 1,098,991.50 pesos cuando todas las demás variables en el modelo son cero.
el coeficiente "sexo_numerica" es de -227,518.30.es decir, las mujeres ganan aproximadamente 227,518.30 menos que los hombres en sueldo El coeficiente es estadísticamente significativo (p < 0.001) por lo que la diferencia salarial entre hombres y mujeres es significativa después de controlar otras variables en el modelo.
El coeficiente asociado con la variable que trata sobre si trabajo sin considerar quehaceres del hogar es de -41,184.94 pesos, pero el valor p correspondiente es 0.341. Por lo que la relación entre esta variable y el salario no es estadísticamente significativa.
Ademas, posee un R2 de 0,027, lo que indica que solo explicaria un 27% de la variabilidad en el ingreso.
